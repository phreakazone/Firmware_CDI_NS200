                           ...
                           char *out, size_t out_size)
{
    ...
    if (!strcmp(op,"PICKUP")) {
        ...
        if (!setup_can_write(p)) return error_frame(seq,"REMOVE_JP_HV_STOP_ENGINE",out,out_size);
    ...
    if (!strcmp(op,"STROBE")) {
        ...
        if (!strcmp(x,"ON")) { if (p->store->setup.stage<CDI_R7_STAGE_PICKUP_OK||p->hv_enabled) return error_frame(seq,"PICKUP_OR_JP_HV",out,out_size); p->strobe_active=true; p->strobe_samples=0; }
    ...
    if (!strcmp(op,"MANUAL_TDC") && parse_uint(strtok_r(NULL,",",save),35999u,&a)) {
        ...
        if (!setup_can_write(p)||p->store->setup.stage<CDI_R7_STAGE_PICKUP_OK) return error_frame(seq,"PICKUP_OR_JP_HV",out,out_size);
    ...
    if (!strcmp(op,"FIRST_START")) {
        if (!setup_can_write(p)||p->store->setup.stage<CDI_R7_STAGE_TDC_SAVED) return error_frame(seq,"TDC_OR_JP_HV",out,out_size);
        ...
        p->store->setup.fan_mode=CDI_R7_FAN_ON; p->first_start_seconds=0;
        if (!persist_store(p)){p->store->setup=old; return error_frame(seq,"FLASH",out,out_size);}
    ...
    if (!strcmp(op,"READY")) {
        ...
        bool first_proven=p->store->setup.stage==CDI_R7_STAGE_READY||
            (p->store->setup.stage>=CDI_R7_STAGE_FIRST_START&&p->first_start_seconds>=3u);
    ...
    *star = '\0';
    ...
    if (strcmp(cmd, "GET") == 0) {
        ...
        if (what != NULL && strcmp(what, "STATUS") == 0) {
            n = snprintf(body, sizeof(body),
                ...
                p->physical_arm ? 1u : 0u, p->pro_jumper ? 1u : 0u);
    ...
    if (strcmp(cmd, "LIVE") == 0 &&
        ...
        cdi_r5_status_t s = cdi_r5_live_set_cell(&p->working, (uint8_t)a,
            (uint8_t)b, (int16_t)c, p->rpm != 0u, p->pro_jumper);
    ...
    if (strcmp(cmd, "LIMIT") == 0) {
        ...
        if (cdi_r5_map_validate(&candidate, p->pro_jumper) != CDI_R5_OK)
    ...
    if ((strcmp(cmd, "LOAD") == 0 || strcmp(cmd, "SAVE") == 0) &&
        ...
        if (strcmp(cmd, "LOAD") == 0) {
            uint8_t old_active;
            if (cdi_r5_map_validate(&p->store->slots[a], p->pro_jumper) != CDI_R5_OK)
